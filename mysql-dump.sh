...
LOGFILENAME=$LOG_PATH/mydumpadmin.log
...
delete_old_backups() {
        ...
        ${FIND} ${LOCAL_BACKUP_DIR} -mtime +$DELETE_DAYS -exec rm -f {} \;
}
...
rsync() {
        ${RSYNC} --delete --progress -avz ${LOCAL_BACKUP_DIR} ${RSYNC_USERNAME}@${RSYNC_HOST}:${RSYNC_UPLOAD_DIR}
}
...
db_backup
send_report
[ $DELETE_ENABLE -eq 1 ] && delete_old_backups
[ $RSYNC_ENABLE -eq 1 ] && rsync
